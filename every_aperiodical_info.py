...
def get_finance_forecast(code, period_start_date, period_end_date):
    # 预告类型、利润下限、利润上限、去年同期利润、利润变幅下限、利润变幅上限
    fields = ["type", "profit_min", "profit_max", "profit_last", "profit_ratio_min", "profit_ratio_max", "content"]
    table = jq.finance.STK_FIN_FORCAST
    cond1 = jq.finance.STK_FIN_FORCAST.code == code
    cond2 = jq.finance.STK_FIN_FORCAST.pub_date <= period_end_date
    cond3 = jq.finance.STK_FIN_FORCAST.pub_date >= period_start_date
    order = jq.finance.STK_FIN_FORCAST.pub_date.desc()
    query = jq.query(table).filter(cond1, cond2, cond3).order_by(order)
    data_df = jq.finance.run_query(query)
    if data_df.empty:
        return pd.DataFrame({field: [] for field in ["security"] + fields + ["start_date", "end_date"]})
